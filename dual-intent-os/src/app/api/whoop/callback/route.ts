...
export async function GET(req: NextRequest) {
  ...
  if (oauthError || !code) {
    ...
    return NextResponse.redirect(
      `${origin}/dashboard?whoop=error&reason=${encodeURIComponent(reason)}`
    );
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return NextResponse.redirect(
        `${origin}/dashboard?whoop=error&reason=token_exchange`
      );
    }
    ...
    return NextResponse.redirect(`${origin}/dashboard?whoop=error&reason=network`);
  }
